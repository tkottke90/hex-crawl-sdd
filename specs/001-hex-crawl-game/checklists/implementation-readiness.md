...
- [X] CHK023 — Is the "combat ends when side is eliminated" condition specified in T041 or T045 — specifically, does "eliminated" mean all dead/incapacitated or all dead only (given Casual-mode incapacitation)? [Clarity, tasks.md:T041/T045, Ambiguity] — **Patched**: T041 now provides `isCombatOver()` specifying that eliminated = all units on a side have `status === 'dead'`. `'incapacitated'` does not exist — single terminal status in all modes.
...
- [X] CHK032 — Does T073 (`RunEndDetector.ts`) specify the exact condition for Casual mode — "party never ends in Casual" is a business rule; is this documented in the task or assumed from spec? [Clarity, tasks.md:T073] — **Resolved**: T073 now explicitly states `checkRunEnd()` returns `true` if any character with `role === 'pc'` OR `role === 'escort'` has `status === 'dead'`, and notes "This applies in **both Casual and Roguelike modes** — mode does NOT change the run-end condition, only save behavior."
...
- [X] CHK051 — Do T010/T018 (model files) specify that `Character` now requires `role: CharacterRole`, `deathRecord: DeathRecord | null` — or do the task files still reference the pre-clarification model shape? [Completeness, tasks.md:T010, data-model.md, Gap] — **Resolved**: T010 explicitly lists `CharacterRole`, `DeathRecord` in `src/models/character.ts` and boldly notes `role: CharacterRole` and `deathRecord: DeathRecord | null` are required; `CharacterStatus = 'active' | 'dead'` only.
- [X] CHK052 — Is there a task (or sub-task of T010) that creates the `CharacterRole` type (`'pc' | 'escort' | 'adventurer'`) and `DeathRecord` interface — or are these missing from the task breakdown entirely? [Completeness, tasks.md:T010, Gap] — **Resolved**: T010 explicitly enumerates `CharacterRole` and `DeathRecord` as interfaces in `src/models/character.ts`. T019 creates `CharacterRoleSchema = z.enum(['pc','escort','adventurer'])` and `DeathRecordSchema` in Zod schemas.
- [X] CHK053 — Does any task specify how the starting party (exactly 1 PC + 1 Escort) is constructed at new-game creation — is this covered in T033/T034 (MainMenu/WorldMap init) or is it a missing task? [Completeness, tasks.md:T033/T034, Gap] — **Resolved**: T034 explicitly constructs the starting party: "1 PC (player-chosen class, `role: 'pc'`, `recruitmentSource: 'starting'`) and 1 Escort (pre-authored template from `src/data/escort.ts`, `role: 'escort'`, `recruitmentSource: 'starting'`)." T030 creates the escort template.
- [X] CHK054 — Do T063 (`Serialiser.ts`) and T065 (`IndexedDbStore.ts`) tasks specify that `SaveState` now has `deathHistory: DeathRecord[]` and `invalidated: boolean` — are these new fields visible in the task descriptions? [Completeness, tasks.md:T063/T065, Gap] — **Resolved**: T010 declares `SaveState` with `deathHistory: DeathRecord[]` and `invalidated: boolean`. T019 includes `z.array(DeathRecordSchema)` and `z.boolean()`. T063 repeats both fields in bold. T015 asserts `deathHistory: []` and `invalidated: false` on fresh state.
- [X] CHK055 — Does T073 (`RunEndDetector.ts`) task text specify that run-end now triggers on **PC or Escort death** (via `character.role`) in **both modes** — not "Roguelike only" and not "all dead"? [Clarity, tasks.md:T073, Spec §FR-013/FR-014] — **Resolved**: T073 explicitly states `role === 'pc' OR role === 'escort'` check, applies "in **both Casual and Roguelike modes**", and notes "mode does NOT change the run-end condition, only save behavior."
- [X] CHK056 — Does T039 (`mode-rules.test.ts`) test task assertion reflect that `CharacterStatus` no longer includes `'incapacitated'` — specifically, has the test expectation been updated from `status: 'incapacitated'` (Casual) to `status: 'dead'` (all modes)? [Consistency, tasks.md:T039, data-model.md] — **Resolved**: T039 explicitly states "Both Casual and Roguelike → character `status` becomes `'dead'` at 0 HP (no `incapacitated` state)" and asserts `status: 'dead'` in both mode variants.
- [X] CHK057 — Does T044 (`ModeRules.ts`) task specify the new death semantics — all characters set to `status: 'dead'`; mode difference is purely in save invalidation/reload behavior — or does it still reference the old `incapacitated` model? [Consistency, tasks.md:T044, Spec §FR-013] — **Resolved**: T044 now specifies `applyDefeat(character: Character, mode: GameMode, coord: HexCoord, turn: number): Character` returning `status: 'dead'` in both modes, with mode difference in save behavior only. No `incapacitated` reference.
- [X] CHK058 — Is there a task specifying when and how `save.invalidated = true` is set in Roguelike mode (e.g., in T073, T076, or a new task in Phase 7) — or is this field created in the data model but never set in the task plan? [Completeness, tasks.md §Phase 7, Gap] — **Resolved**: T073a implements `invalidateSave(saveState, mode): SaveState` returning `invalidated: true` when `mode.type === 'roguelike'`. T076 wires `Combat.ts` to call `invalidateSave()` after `checkRunEnd()` returns `true`.
